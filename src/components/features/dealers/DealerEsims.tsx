...
function DealerEsims({
  ...
  return (
    <GenericDataTable
      data={data.data.esims_purchased.data}
      columns={DealerEsimColumns}
      pageCount={data.data.esims_purchased.pagination.totalPages}
      initialState={{
        // sorting: [{ id: "amount", desc: true }],
        columnPinning: { right: ["actions"] },
        pagination: {
          pageIndex: 1,
          pageSize: data.data.esims_purchased.pagination.limit,
        },
      }}
      isLoading={isFetching}
      exportTitle="Dealer eSIMs"
      exportFilename="dealerEsims"
      applyFilter={false}
    />
  );
}

export default DealerEsims;
