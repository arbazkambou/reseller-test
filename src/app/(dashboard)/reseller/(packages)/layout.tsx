...
import { resellerDataOnlyLinks, resellerDataVoiceLinks } from "@/lib/data/data";
...
export default function PackagesLayout({
  ...
  const esimLinks = pathname.startsWith("/reseller/data-only-esim/")
        ...
        <CardContent className="flex flex-col gap-12">
          <section className="grid w-full  gap-[2.31rem] xl:grid-cols-[auto_1fr_auto]  xl:gap-[4rem]">
            <div className="flex items-center justify-center gap-[1.5rem] md:gap-[2rem] xl:order-2 xl:justify-end xl:gap-[1rem]">
              <Link href={"/reseller/data-only-esim"}>
                <PackageNavigationButton
                  isActive={pathname.startsWith("/reseller/data-only-esim")}
                >
                  Data Only
                </PackageNavigationButton>
              </Link>
              <Link href={"/reseller/data-voice-esim"}>
                <PackageNavigationButton
                  isActive={pathname.startsWith("/reseller/data-voice-esim")}
                >
                  Data Voice
                </PackageNavigationButton>
              </Link>
